# 7TVEmoteDownloader
A little script that downloads every emote of a 7TV User/Streamer using webscraping (chromedriver).
...
Put the chromedriver executable in the root directory and set the path (when asked) to 
```bash
...
```bash
mv ./target/yourtarget/release/SevenTVEmoteDownloader .
./SevenTVEmoteDownloader
```
